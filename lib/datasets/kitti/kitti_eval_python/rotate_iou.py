...
try:
    ...
    cuda.gpus.lst  # triggers driver init
except Exception:
    # No CUDA device available – create a dummy decorator so the module can
    # still be imported (GPU eval functions will not work, but that is fine
    # for scripts that only need the dataset/model without evaluation).
    class _DummyCuda:
        @staticmethod
        def jit(*args, **kwargs):
            def wrapper(func):
