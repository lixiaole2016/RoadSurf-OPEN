...
def get_calib_from_file(calib_file):
    ...
    # Parse calibration parameters by looking for specific labels
    calib_data = {}
    
    for line in lines:
        line = line.strip()
        if line.startswith('P2:'):
            obj = line.split()[1:]  # Skip the "P2:" label
            calib_data['P2'] = np.array(obj, dtype=np.float32)
        elif line.startswith('P3:'):
            obj = line.split()[1:]  # Skip the "P3:" label
            calib_data['P3'] = np.array(obj, dtype=np.float32)
        elif line.startswith('R0_rect:'):
            obj = line.split()[1:]  # Skip the "R0_rect:" label
            calib_data['R0'] = np.array(obj, dtype=np.float32)
        elif line.startswith('Tr_velo_to_cam:'):
            obj = line.split()[1:]  # Skip the "Tr_velo_to_cam:" label
            calib_data['Tr_velo2cam'] = np.array(obj, dtype=np.float32)
    
    # Use defaults if any matrix is missing
    if 'P2' not in calib_data:
        # print("WARNING: P2 matrix not found, using default values")
        calib_data['P2'] = np.array([721.5377, 0.0, 609.5593, 44.85728, 0.0, 721.5377, 172.854, 0.2163791, 0.0, 0.0, 1.0, 0.002745884], dtype=np.float32)
    
    if 'P3' not in calib_data:
        # print("WARNING: P3 matrix not found, using identity")
        calib_data['P3'] = np.eye(4, dtype=np.float32)[:3].flatten()
    
    if 'R0' not in calib_data:
        # print("WARNING: R0 matrix not found, using identity")
        calib_data['R0'] = np.eye(3, dtype=np.float32).flatten()
    
    if 'Tr_velo2cam' not in calib_data:
        # print("WARNING: Tr_velo_to_cam matrix not found, using identity")
        calib_data['Tr_velo2cam'] = np.eye(4, dtype=np.float32)[:3].flatten()

    return {'P2': calib_data['P2'].reshape(3, 4),
            'P3': calib_data['P3'].reshape(3, 4),
            'R0': calib_data['R0'].reshape(3, 3),
            'Tr_velo2cam': calib_data['Tr_velo2cam'].reshape(3, 4)}
